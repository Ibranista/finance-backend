...
import { Keywords, LinkMarket, Links } from '../../../resources';

const AddOrder = ({ record: initialRecord, resource }) => {
  ...
  const [items] = useState(
    (record.params &&
      flat.unflatten(record.params).items &&
      flat.unflatten(record.params).items) ||
      []
  );
  ...
  const handleAddNewItem = (e = undefined) => {
    e && e.preventDefault();
  ...
  const handleFetchKeywords = () => {
    api
      ...
      .then(({ data }) => {
        const keywordOptions = data.records.map(({ title, id }) =>
          title
            ? {
                label: title,
                value: title
  ...
  const handleAddNewFormValues = () => {
    ...
    const { items } = initialRecordValues;
    const flatParamValues = flat.flatten(initialRecordValues);
    console.log('flatValues', flatParamValues);
    setFormValues({ items: items, ...flatParamValues });
    setRecord({ ...record, params: flatParamValues });
  };
                  ...
                  <Box width="95%" mb={mb} key={index}>
                    ...
                    <Section>
                      ...
                      {!item.filePath && (
                        ...
                        >
                          <Label required={item.contentOrder}>
                      ...
                      {!contentlessLinkTypes.includes(item.linkType) &&
                        resource.properties['items.contentOrder.language'] && (
                          <FormGroup
                            mb={mb}
                            error={
                              errors &&
                        ...
                        resource.properties[
                          ...
                          <FormGroup
                            ...
                            error={
                              ...
                              errors[`items.${index}.contentOrder.sub_keywords`]
                            }
                          >
                            <Label>
                              {(resource.properties[
                            ...
                            <Select
                              ...
                              value={
                                item.contentOrder &&
                                subKeywordOptions.find(
                                  subKeywordOption =>
                                    item.contentOrder &&
                                    item.contentOrder.sub_keywords &&
                                    item.contentOrder.sub_keywords.includes(
                                      subKeywordOption
                                    )
                                )
                              }
