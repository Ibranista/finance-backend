...
const EditPermission = ({ resource, action, record: initialRecord }) => {
  ...
  useEffect(() => {
    if (!fetchingResources && resources.length > 0) {
      const { params } = initialRecord;
      if (params.resource)
        setSelectedResource(
          resources.find(resource => resource.value === params.resource)
        );
      if (params.possession)
        setSelectedPossession(
          possessions.find(possession => possession.value === params.possession)
        );
    }
  ...
  useEffect(() => {
    if (!fetchingResources && allowedActions.length > 0) {
      ...
      if (params.action)
        setSelectedAction(
          allowedActions.find(action => action.value === params.action)
        );
      if (params.allowAllAttributes)
        setAllowAllAttributes(params.allowAllAttributes);
