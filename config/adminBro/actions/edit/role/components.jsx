...
import { mapParamsToStringArray, removeParams } from '../../../utils';

const EditRole = ({ resource, action, record: initialRecord }) => {
  ...
  useEffect(() => {
    if (!fetchingPermissions && permissions.length > 0) {
      const { params } = initialRecord;
      const selectedPermissions = mapParamsToStringArray(
        params,
        /permissions.(\d+)/i
      );
